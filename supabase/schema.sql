...
-- ── Room Players ──────────────────────────────────────────────────────────
-- Persists who is seated at each table and their current chip stack.
-- Survives server cold-starts so players are automatically re-seated.
...
CREATE POLICY "service_role full access" ON casino_room_players USING (TRUE) WITH CHECK (TRUE);
CREATE POLICY "anon read room_players" ON casino_room_players FOR SELECT USING (TRUE);
